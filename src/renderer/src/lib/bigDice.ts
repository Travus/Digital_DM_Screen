...
export const MODES = ['flat', 'advantage', 'disadvantage'] as const
...
export interface Throw {
  ...
  value: number
  /** Both d20s when a mode discarded one of them, else null. */
  pair: Pair | null
}
...
/**
 * Which of a pair the roll keeps, and which it discards.
 *
 * `index` rather than a value, because a pair of equal dice still has a
 * discarded one on screen and `20 vs 20` gives no way to tell them apart. The
 * second is the one dropped in that case, arbitrarily but consistently: the
 * alternative is a strike-through that moves between two identical numbers.
 */
...
export function throwDice(sides: Sides, mode: Mode, zeroIsHundred: boolean): Throw {
  ...
  if (!modeApplies(sides) || mode === 'flat') return { value: rollDie(sides), pair: null }
...
export function slots(sides: Sides, mode: Mode, value: number | null, pair: Pair | null): Slot[] {
  ...
  // and for a layout saved in flat mode — the stage is one die, so switching to
  // advantage does not make a second die appear resting on nothing.
  if (!modeApplies(sides) || mode === 'flat' || pair === null) {
    return [{ solid, value, discarded: false }]
  }

  const kept = keptIndex(pair, mode)
  return [
    { solid, value: pair[0], discarded: kept !== 0 },
    { solid, value: pair[1], discarded: kept !== 1 }
  ]
}
...
export function historyTitle(sides: number, mode: Mode | undefined): string {
  ...
  return mode !== undefined && mode !== 'flat' && modeApplies(sides) ? `${name} · ${mode}` : name
}
